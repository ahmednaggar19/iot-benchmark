...
package cn.edu.tsinghua.iot.benchmark.rate.limiter.influx;
...
public class InfluxDB implements IDatabase {
  ...
  private static String CREATE_URL = "http://%s/api/v2/write?org=%s&bucket=%s&precision=%s";
  ...
  public InfluxDB(DBConfig dbConfig) {
    ...
    String portString = dbConfig.getPORT().get(0);
  ...
  public Status insertOneBatch(IBatch batch) {
    try {
      ...
      String joinedRequests = String.join("\n", lines);
      boolean insert = rateLimiterClient.insert(Collections.singletonList(joinedRequests));
